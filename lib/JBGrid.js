...
class JBGrid extends React.Component {
    ...
    render() {
        ...
        return (
            <div className={"jb-grid-wrapper " + (this.props.className ?? "")} ref={(dom) => vm.JBGridComponentDom = dom} style={this.props.style}>
                <section key={'jb-grid-header'} className="jb-grid-header">
                    <section className={"main-section " + (vm.config.states.headerSection == "main" ? "" : "hidden")}>
                        ...
                        <section className="button-container">
                            {
                                vm.config.customComponents.headerEnd.map((Component, index) => {
                    ...
                    <section className={"search-section " + (vm.config.states.headerSection == "search" ? "" : "hidden")} style={{ height: vm.styles.searchBar.height }}>
                ...
                <section key={'jb-grid-content'} className="jb-grid-content" style={{ height: vm.styles.contentWrapper.height }}>
                    {
                        vm.isErrorOccurred && (
                            <div className="error-panel">
                                <div className="error-image">😬😓🤔</div>
                                <div className="error-text" style={{ padding: " 0 0 0 0" }}>すみません</div>
                                <div className="error-text">{vm.config.captions.serverErrorText}</div>
                                <div className="error-button"><button onClick={(e) => vm.refreshBtnClick(e)}>تلاش مجدد</button></div>
                            </div>
                        )}
                    {
                        !vm.isErrorOccurred &&
                        [
                            <section className="table-header" key='table-header'>
                                <div className="table-header-wrapper" style={{ ...vm.styles.table.generalCols, ...vm.styles.table.scrollIndent }}>
                                    {
                                        vm.config.table.columns.map((item, index) => {
                                            return (
                                                <div onClick={(e) => vm.setSortColumn(item)} key={item.name + '-' + item.id + '-' + "jb-grid-table-header"} title={item.name} className={'header-item ' + (item.sortable ? 'sortable-col' : '')}>
                    ...
                    <section className="btn-wrapper-section">
                        {
                            this.props.isFullscreen !== null && this.props.isFullscreen !== undefined && (
                                <div className="btn" onClick={() => vm.onFullScreenBtnClicked(this.props.isFullscreen)}>
                        ...
                        <div className="btn refresh-btn" onClick={(e) => vm.refreshBtnClick(e)}>
                            <svg id="Capa_1" x="0px" y="0px" viewBox="0 0 305.836 305.836" space="preserve" ref={vm.elements.refreshIcon}>
                                <g>
                    ...
                    <section className="page-section">
                        <section className="page-size-section" title="تعداد آیتم در هر صفحه">
                            <select value={vm.config.page.size} className="page-size-select" onChange={(e) => vm.onPageSizeChange(e)}>
                                <option value={20}>{vm.toPersianNumber(20)}</option>
                                <option value={30}>{vm.toPersianNumber(30)}</option>
                                <option value={50}>{vm.toPersianNumber(50)}</option>
                                <option value={100}>{vm.toPersianNumber(100)}</option>
                            </select>
                        </section>
                        <section className="items-information-section">
                            <span>{vm.toPersianNumber(vm.config.data.itemsInformation.startItemIndex)}</span>
                            <span>-</span>
                            <span> {vm.toPersianNumber(vm.config.data.itemsInformation.endItemIndex)} </span>
                            <span> &nbsp;از</span>
                            <span title="تعداد کل ایتم های موجود">&nbsp; {vm.toPersianNumber(vm.config.data.itemsInformation.totalItemsCount)} &nbsp;</span>

                        </section>
                        <section className="navigation-section nav-btn">
                            <nav>
                                <div className="last-Page-btn nav-btn" title={vm.config.page.totalPages} onClick={(e) => vm.goToLastPage(e)}>
                                    <svg width="100%" height="100%" viewBox="0 0 24 24" fit="" preserveAspectRatio="xMidYMid meet" ><path d="M15 6 v12 h2 v-12 h-2z M8 6L6.59 7.41 11.17 12l-4.58 4.59L8 18l6-6z"></path></svg>

                                </div>
                                <div className="next-Page-btn nav-btn" onClick={(e) => vm.goToNextPage(e)}>
                                    <svg width="100%" height="100%" viewBox="0 0 24 24" fit="" preserveAspectRatio="xMidYMid meet" ><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"></path></svg>

                                </div>
                                <div className="number-container">
                                    <div className="number-wrapper">
                                        <div className="number-btn" onClick={() => vm.goToPage(vm.config.page.index + 2)}>{vm.config.page.index + 2 <= vm.config.page.totalPages ? vm.toPersianNumber(vm.config.page.index + 2) : ''}</div>
                                        <div className="number-btn" onClick={() => vm.goToPage(vm.config.page.index + 1)}>{vm.config.page.index + 1 <= vm.config.page.totalPages ? vm.toPersianNumber(vm.config.page.index + 1) : ''}</div>
                                        <div className="number-btn current-page" onClick={() => vm.changePageNumberToInput()}>{vm.toPersianNumber(vm.config.page.index)}</div>
                                        <div className="number-btn" onClick={() => vm.goToPage(vm.config.page.index - 1)}>{vm.config.page.index - 1 > 0 ? vm.toPersianNumber(vm.config.page.index - 1) : ''}</div>
                                        <div className="number-btn" onClick={() => vm.goToPage(vm.config.page.index - 2)}>{vm.config.page.index - 2 > 0 ? vm.toPersianNumber(vm.config.page.index - 2) : ''}</div>
                                    </div>
                                </div>
                                <div className="prev-Page-btn nav-btn" onClick={(e) => vm.goToPrevPage(e)}>
                                    <svg width="100%" height="100%" viewBox="0 0 24 24" fit="" preserveAspectRatio="xMidYMid meet" ><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path></svg>
                                </div>
                                <div className="first-Page-btn nav-btn" onClick={(e) => vm.goToFirstPage(e)}>
                                    <svg width="100%" height="100%" viewBox="0 0 24 24" fit="" preserveAspectRatio="xMidYMid meet" ><path d="M7 6 v12 h2 v-12 h-2z M17.41 7.41L16 6l-6 6 6 6 1.41-1.41L12.83 12z"></path></svg>

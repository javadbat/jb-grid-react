...
import JBSearchBarData                    from 'jb-searchbar-react/dist/JBSearchBarData$Build_Type_Prefix.min'
class JBGridService{
    ...
    constructor(config,bridge, filterConfig){
        ...
        if(typeof bridge != 'function'){		        this.dataBridge = new bridge();
            console.error('JBGrid need Bridge to perform well');		
          }else{		
            this.dataBridge = new bridge();		
          }
        this.InitGrid(filterConfig);
    }
    InitGrid(filterConfig){
    ...
    initFilter(filterConfig){
        if(filterConfig){
            ...
            this.filter.config = this.mergeObject(filterConfig,defualtFilterConfig);
            this.filter.config.callBacks.onHeightChange = (searchBarHeight)=>{this.searchBarHeight = searchBarHeight;this.setGridElementsHeight()}     
        }
    }
    InitSize(){
    ...
    setGridElementsHeight(){
            //update grid content height base on header and footer height
            var footerHeight = 48;
            var headerHeight = 48;
            ...
            this.styles.contentWrapper.height = `calc(100% - ${totalHeight}px)`;
    ...
    fetchGridData(){
     ...
     onSearch(filterList){
        ...
        var onSeachPromise = new Promise((resolve, reject)=> {
    ...
    openSearchHeaderSection(){
        this.filter.config.triggers.focus();
